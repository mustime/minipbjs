...
var PROTOBUF = 'protobuf', ROOT = '$root', READER_FUNCTIONS = '$reader', WRITER_FUNCTIONS = '$writer', TYPES_ARRAY = '$types', WIRES_ARRAY = '$wires', PACKED_ARRAY = '$packed', CONVERTER_CREATOR = '$createConverter', MESSAGE_CONVERTER = '$conv_message', OBJECT_CONVERTER = '$conv_object', CONVERTER = 'converter', CONVERT_FUNCTION = 'convf', CONFIGS = 'cfgs', CONFIG = 'cfg', RETURN_VALUE = 'ret', VALUE = 'value', VALUES = 'values', TYPE_PLACEHOLDER = '$', ENUM_PLACEHOLDER = '@', CLASS_NAME = 'C', PROPERTIES = 'props', TYPES = 'types', TYPE = 'type', VALUE_TYPE = 'vtype', NAMESPACE = 'namespace', PARENT = 'parent', IS_INNER = 'inner', KEYS = 'keys', KEY = 'key', FIELD_PAYLOADS = 'payloads', FIELD_PAYLOAD = 'payload', READER = 'reader', WRITER = 'writer', LENGTH = 'length', END_POS = 'end', MESSAGE = 'message', TAG = 'tag', FIELD = 'field', FIELD_ID = 'id', WIRE = 'wire', FIELD_MAP = 'map', FIELD_REPEATED = 'array', PATH = 'path', IS_PACKED = 'packed', OBJECT = 'object', PROPERTY = 'property', OPTIONS = 'options';
var DEFAULT_ROOT = 'default';
var NUMBERIC64_ARR = ['int64', 'sint64', 'fixed64', 'sfixed64', 'uint64'];
var config;
function static_mini(root, options, callback) {
    config = options;
    if (config.forceLong && !protobufjs_1.default.util.Long) {
    ...
    var codeGen = new codegen_1.SimpleCodeGen(4);
    // global definition
    codeGen
        .pushComments([
        "DO NOT EDIT",
        "-- this file is generated by minipbjs v" + version_1.VERSION_STR,
    ...
    if (config.decode) {
        ...
        codeGen
            ...
            .pushLine("%s.skip(%s & /*2^3-1=*/7);", READER, TAG).subIndent()
            .pushLine("}").subIndent()
            .pushLine("}")
    ...
    codeGen
        ...
        .pushLine("} else if (%s[%s]['%s'] && Object.values(%s[%s]['%s']).every(function(v) { return Number(v); })) {", NAMESPACE, KEY, ENUM_PLACEHOLDER, NAMESPACE, KEY, ENUM_PLACEHOLDER).addIndent()
        ...
        .pushLine("}")
        .pushLine("return %s;", VALUES).subIndent()
...
function writeNamespace(codeGen, ns, isLastOne) {
    if (!ns.name)
        throw new Error('namespace needs a name');
    codeGen.pushLine("%s: {", ns.name).addIndent();
    // sort before iteration to avoid meanless modification to VCS
    ns.nestedArray.sort(function (a, b) { return a.name.localeCompare(b.name); });
    ns.nestedArray.forEach(function (nested, index, arr) {
        if (nested instanceof protobufjs_1.default.Type) {
            writeType(codeGen, nested, index === arr.length - 1);
        }
        else if (nested instanceof protobufjs_1.default.Enum) {
            writeEnum(codeGen, nested, index === arr.length - 1);
        }
        else if (nested instanceof protobufjs_1.default.Namespace) {
            writeNamespace(codeGen, nested, index === arr.length - 1);
        }
    });
    codeGen.subIndent().pushLine("}%s", isLastOne ? '' : ',');
}
function writeType(codeGen, type, isLastOne) {
    var props = {};
    var maxFieldId = Math.max.apply(Math, Object.keys(type.fieldsById).map(Number));
    // 1-based field id. ignore absent fields.
    for (var id = 1; id <= maxFieldId; id++) {
        var field = type.fieldsById[id];
        if (field) {
            var fieldAttr = [];
            fieldAttr.push(field.name);
            var fieldType = field.resolvedType ? field.resolvedType.fullName : field.type;
            if (field.map) {
    ...
    if (type.nestedArray.length) {
        // sort before iteration to avoid meanless modification to VCS
        type.nestedArray.sort(function (a, b) { return a.name.localeCompare(b.name); });
        type.nestedArray.forEach(function (nested, index, arr) {
            if (nested instanceof protobufjs_1.default.Type) {
                writeType(codeGen, nested, index === arr.length - 1);
            }
            else if (nested instanceof protobufjs_1.default.Enum) {
...
function writeEnum(codeGen, type, isLastOne) {
    codeGen
...
//# sourceMappingURL=static-mini.js.map