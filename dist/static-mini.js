...
var PROTOBUF = 'protobuf', ROOT = '$root', READER_FUNCTIONS = '$reader', WRITER_FUNCTIONS = '$writer', TYPES_ARRAY = '$types', WIRES_ARRAY = '$wires', PACKED_ARRAY = '$packed', READER_SKIP = '$rskip', WRITER_LDELIM = '$wldelim', WRITER_FORK = '$wfork', RW_WRAP = '$', CLASS_NAME = 'C', PROPERTIES = 'props', TYPE = 'type', NAMESPACE = 'namespace', PARENT = 'parent', KEYS = 'keys', KEY = 'key', FIELD_PAYLOADS = 'payloads', FIELD_PAYLOAD = 'payload', READER = 'reader', WRITER = 'writer', LENGTH = 'length', END_POS = 'end', MESSAGE = 'message', TAG = 'tag', FIELD = 'field', FIELD_ID = 'id', WIRE = 'wire', REPEATED_FIELD = 'array', PATH = 'path', IS_PACKED = 'packed';
...
function static_mini(root, options, callback) {
    ...
    codeGen
        ...
        .pushLine(" * -- this file is generated by minipbjs v%s", version_1.VERSION_STR)
        .pushLine(" */ ")
        .pushLine()
...
function writeNamespace(codeGen, ns, isLastOne) {
    if (ns instanceof protobufjs_1.default.Type) {
        var props = {};
        var maxFieldId = Math.max.apply(Math, Object.keys(ns.fieldsById).map(Number));
        // 1-based field id. ignore absent fields.
        for (var id = 1; id <= maxFieldId; id++) {
            var field = ns.fieldsById[id];
            if (field) {
                ...
                var type = field.resolvedType ? field.resolvedType.fullName : field.type;
                if (field.map) {
        ...
        ns.nestedArray.forEach(function (nested, index, arr) {
            if (nested instanceof protobufjs_1.default.Namespace) {
